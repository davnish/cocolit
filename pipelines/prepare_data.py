...
from src.vec_to_labels import vectors_to_labels
...
def PrepareDataset(
    ...
    try:
        ...
        PatchRaster(
            raster_path,
            output_patched_ras=raster_patch_path,
            patch_size=640,
            padding=True,
        )
        ...
        PatchVector(
            raster_patch_path, vector_path, output_patched_vector=vector_patch_path
        )
        ...
        trainx, trainy, testx, testy = train_test_split(
            raster_patch_path, txt_patch_path, train_size=0.8
        )
        testx, testy, valx, valy = train_test_split(
            raster_patch_path, txt_patch_path, train_size=0.5
        )
        logging.info(
            f"Train and test data split. Train size: {len(trainx)}, Test size: {len(testx)}, Val size: {len(valx)}"
        )
