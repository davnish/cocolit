...
with st.container(height=400, border=False):
    ...
    for idx, col in enumerate(cols):
        with col:
            ...
            st.button('Yes', key=f"{idx}_2", use_container_width=True, type='primary')
            st.button('No', key=f"{idx}_1", use_container_width=True)
...
with st.container():
    ...
    for idx, mcol in enumerate(mapcols):
        ...
        with mcol:
            ...
            pt.add_child(trees)
            st_folium(m, use_container_width=True, feature_group_to_add=pt, height = height, key=idx)
